...
def test_non_equilibrium_thermochemical_error_with_multiple_X_points(datasets_db):
    ...
    error, gradient = calculate_non_equilibrium_thermochemical_probability(dbf, thermochemical_data)
...
def test_subsystem_non_equilibrium_thermochemcial_probability(datasets_db):
    ...
    bin_prob, bin_prob_gradient = calculate_non_equilibrium_thermochemical_probability(dbf_bin, thermochemical_data)

    # Getting binary subsystem data explictly (from binary input)
    thermochemical_data = get_thermochemical_data(dbf_tern, ['CR', 'NI', 'VA'], phases, datasets_db)
    prob, prob_gradient = calculate_non_equilibrium_thermochemical_probability(dbf_tern, thermochemical_data)
    assert np.isclose(prob, bin_prob)
...
def test_non_equilibrium_thermochemcial_species(datasets_db):
    ...
    prob, gradient = calculate_non_equilibrium_thermochemical_probability(dbf, thermochemical_data)
